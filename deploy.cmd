...
call :ExecuteCmd nuget.exe restore "%DEPLOYMENT_SOURCE%\WebApplication1\WebApplication1.sln" -packagesavemode nuspec
IF !ERRORLEVEL! NEQ 0 goto error
...
:: 3. KuduSync
call :ExecuteCmd "%KUDU_SYNC_CMD%" -v 50 -f "%DEPLOYMENT_TEMP%/API" -t "%DEPLOYMENT_TARGET%" -n "%NEXT_MANIFEST_PATH%" -p "%PREVIOUS_MANIFEST_PATH%" -i ".git;.hg;.deployment;deploy.cmd"
IF !ERRORLEVEL! NEQ 0 goto error
